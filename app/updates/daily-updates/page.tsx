...
export default function DailyUpdatesPage() {
    ...
    const [editingItem, setEditingItem] = useState<DailyUpdate | null>(null);
    ...
    const handleEdit = (item: DataItem) => {
        // Convert DataItem to DailyUpdate
    ...
    const handleSubmit = async (formData: { title: string; description: string }) => {
        try {
            const url = editingItem
                ? `/api/daily-updates/${editingItem.id}`
                : "/api/daily-updates";
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-fd-foreground">Daily Updates</h1>
                    <p className="mt-1 text-sm text-fd-muted-foreground">
                        Manage your daily work updates
                    </p>
                </div>
                <button
                    onClick={handleAdd}
            ...
            <DataFormModal
                ...
                title={editingItem ? "Edit Daily Update" : "Add Daily Update"}
            />
        </div>
    );
}
